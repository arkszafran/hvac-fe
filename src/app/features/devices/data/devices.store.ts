...
import { DevicesApi, DevicesListResponseDto, PaginationResponseDto } from '../../../common/api';
...
export class DevicesStore {
  ...
  private readonly requestSubject = new Subject<DevicesListRequest>();
  private readonly queryState = signal('');
  private readonly pageState = signal(1);
  private readonly itemsState = signal<Device[]>([]);
  ...
  readonly devices = computed(() => {
    ...
    return this.filteringMode() === 'client'
      ? items.filter((device) => matchesDeviceSearch(device, this.query()))
      : items;
  });
  ...
  private applyResponse(response: DevicesListResponseDto): void {
    this.hasLoadedState.set(true);
    this.hasErrorState.set(false);
